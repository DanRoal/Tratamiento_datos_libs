...
#vamos a restarle a cada valor de la intensidad de la luz el valor promedio
#de la intensidad de la luz y después dividirlo entre la desviación estándar

def normalizar(diccionario):
    valores = diccionario.values()
    promedio = np.mean(list(valores))
    desviacion = np.std(list(valores))
    for lamda in diccionario:
        diccionario[lamda] = (diccionario[lamda] - promedio)/desviacion
    return diccionario
...
def limpiar_directorio(directorio):
    ...
    os.rename('promedio.csv', f'{directorio}_promedio_nom1.csv')
